...
- تمامی پورت های پیش فرض برداشته شد. باید خودتان وارد نمایید.
- برای haproxy، پورتی به نام haproxy port اضافه شده که قبلا به صورت پیش فرض 443 بود اما الان باید خودتان، پورت مورد نظرتان را وارد نمایید.(در آخر کانفیگ ادرس و پورت به شما نمایش داده میشود)
- برای پینگ تانل، پورتی به نام local port اضافه شده که قبلا به صورت پیش فرض 443 بود اما الان باید خودتان، پورت مورد نظرتان را وارد نمایید. ( این پورت، پورت جدید کانفیگ شما خواهد بود و در آخر کانفیگ هم به شما نمایش داده میشود)
-----------
...
----------------
```
sudo apt-get install python3 -y && apt-get install wget -y && apt-get install python3-pip -y && pip3 install colorama && pip3 install netifaces && apt-get install curl -y && python3 <(curl -Ls https://raw.githubusercontent.com/FTune12/ICM/main/icmp.py --ipv4)
```
...
```
python3 <(curl -Ls https://raw.githubusercontent.com/FTune12/ICM/main/icmp.py --ipv4)
```
--------------------------------------
 <div dir="rtl">&bull; اگر سرور شما خطای externally-managed-environment داد از دستور زیر اقدام به اجرای اسکریپت نمایید.</div>
 
```
bash -c "$(curl -fsSL https://raw.githubusercontent.com/FTune12/ICM/main/managed.sh)"
```
